...
$(document).ready(function() {
	...
	$('#btn_reg').bind('click', function() {
		$('#form_reg').addClass('vrf');
		if(!$('#form_reg')[0].checkValidity()) return;
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'member/doReg',
			cache: false,
			async : false,
	...
	$('#btn_login').bind('click', function() {
		$('#form_login').addClass('vrf');
		if(!$('#form_login')[0].checkValidity()) return;
		$.ajax({
			type: "POST",
			...
			complete: function(response){
				switch(response.responseText) {
					case '0':
						...
						break;
					case '1':
						window.location = getParameterByName('back');
						break;
				}
			}
		});
...
function onkuPass() {
	$('#form_reg input[name=txt_checkpassword]').attr('pattern', $('#form_reg input[name=txt_password]').val());
}
