	...
	$info[0]['status'] == PROJECT_STATUS_2_SUCCESS
		...
		<div class="tab-pane" id="step_2">
			<ul class="nav nav-tabs">
				...
				<li class="pull-right">
<?php if(($info[0]['status'] == PROJECT_STATUS_1_SUCCESS) || ($info[0]['status'] == PROJECT_STATUS_2_FAIL)): ?>					
					<div class='col-sm-6'>
						<div class="btn btn-block btn btn-danger" id='step_2_send'>第二階段資料送審</div>
					</div>
					<div class='col-sm-6'>
			...
			<div class='tab-content'>
				<div class="tab-pane active form-horizontal" id="step_2_1">
					...
					<form id='form_2_1' class='vrf'>
						...
						<div class='form-group'>
							<label for="inputEmail3" class="col-sm-2 control-label">計畫詳情</label>
							<div class="col-sm-10">
 								<textarea id="summernote" name="txt_project_detail" rows="5" cols="110" width='560px' class="textarea" required><?= $info[0]['detail'] ?></textarea>
							</div>
						</div>
					</form>
				</div>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									...
									<h4 class="modal-title" id="modal_add_productLabel">新增贊助方案</h4>
								</div>
								<div class="modal-body row">
									<form id='form_add_product' class='vrf'>
										...
										<div class="form-group">
											<label for="inputEmail3" class="col-sm-3 control-label">方案圖片<br>700 X 350<br>2M以內</label>
										...
										<div class="form-group">
											<label for="inputEmail3" class="col-sm-3 control-label">前台顯示限量個數</label>
											<div class="col-sm-9">
												<input type="number" class="form-control" name="txt_show_limit" required max="30000">
											</div>		
										</div>
									</form>
								</div>
					...
					<div class="modal fade" id="modal_2_3" tabindex="-1" role="dialog" aria-labelledby="modal_2_3Label">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
									<h4 class="modal-title" id="modal_2_3Label">新增常見問題</h4>
								</div>
								<div class="modal-body row">
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
									<button id='btn_save_2_3' type="button" class="btn btn-primary">Save changes</button>
								</div>
							</div>
						</div>
					</div>						
...
$(function() {
	$('#step_2_send').bind('click', function() {
		if(!confirm('是否確定送審資料？')) return false;
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'plist/editProjectSattus',
			cache: false,
			async : false,
	...
	$('#tblmain').bootstrapTable({
		...
		columns: [{
			...
			formatter: function(obj) {
				var ret = '';
				for (var i = 0; i < obj.length; i++) {
					ret += (ret.length > 0 ? "<br>" : '') + obj[i].show_limit;
				}
				return ret;
			},				
			width:"10%",
...
function editFileinput(url_pic, url_pic_web) {
	$('#file_picture').fileinput({
		maxFileCount : 1,
		showUpload: false,
		showRemove: false,
		...
		uploadUrl: $('#hid_baseurl').val() + "upload/quick",
		allowedFileExtensions: ["jpg", "gif", "png", "jpeg"],
		initialCaption: url_pic,
        initialPreview: [
            "<img class='kv-preview-data krajee-init-preview file-preview-image' width='334px' src='"+ url_pic_web +"'>"
        ],
        uploadExtraData: function (previewId, index) {
			var info = {'type': 'qmaker' };
			return info;
...
function modalUnLock(modal_id) {
	...
	$('input[name=txt_show_limit]').attr('type', 'number');
...
function modalSet(modal_id, row) {
	switch(modal_id) {
		case 'modal_add_product':
			...
			$('input[name=txt_show_limit]').val(row.show_limit);
			break;
		case 'modal_edit_product':
			$('input[name=txt_name]').val(row.name);
			$('textarea[name=txt_detail]').text(row.detail);
			$('input[name=txt_price_origin]').val(row.price_origin);
			$('input[name=hid_url_pic]').val(row.url_pic);
			var url_pic_web = <?= "'".URL_GOOGLE_IMG."'" ?> + row.url_pic;
			...
			$('#file_picture').fileinput('destroy');
			editFileinput(row.url_pic, url_pic_web);
			break;
		case 'modal_2_3':
...
window.operateEvents = {
	...
	'click .medit': function (e, value, row, index) {
		modalSet('modal_edit_product', row);
		$('#modal_add_product').modal('show');
	},
	'click .mdelete': function (e, value, row, index) {
		...
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'plist/doDelProduct',
			cache: false,
			async : false,
			data: {id: row.id},
			error: function(xhr){
