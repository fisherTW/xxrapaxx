...
<?php if($ary_data[0]['status_send'] == '3' && empty($ary_refund)): ?>
				<div class="col-xs-9">
					<input type="text" class="form-control" id="refund_amt" name="txt_amt" placeholder="請輸入實際退款金額" pattern="(\d{1,5})?$" maxlength="5" required />
				</div>
				<div class="col-xs-3"">
					<button class="btn btn-danger btn-block" id="btn_refund">確認退款</button>
				</div>
<?php endif; ?>
<?php if($ary_data[0]['status_send'] == '3' && !empty($ary_refund)): ?>
...
$(function() {
	...
	$('#btn_refund').bind('click', function() {
		if(!$('#refund_amt')[0].checkValidity()) return;

		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'refund/doRefund',
			cache: false,
			async : false,
			...
			complete: function(response){
				location.href = $('#hid_baseurl').val() + 'refund/edit_qgoods/'+$('input[name=hid_order_id').val()+'/'+$('input[name=hid_store_id').val()
			}
		});
		return false;
	});
