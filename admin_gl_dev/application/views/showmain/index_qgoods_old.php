...
<section class="content">
	...
	<input type='hidden' id='row_theme' value=''>
	...
	<form class="form-horizontal vrf" id="form_data">
		<input type='hidden' id='hid_row_theme' name='hid_row_theme' value=''>
...
<?php for ($i=0; $i < count($banner_pdt); $i++) : ?>
					<div class="form-group banner<?= $i+1 ?>">
						...
						<div class="col-sm-5">
							<input type="url" class="form-control" name="txt_url<?= $i+1 ?>" value="<?= $banner_pdt[$i]['url']?>" placeholder="請輸入整串網址" required />
						</div>
						<div class="col-sm-2">
							<img name="banner_pic" src="<?= URL_GOOGLE_IMG.$banner_pdt[$i]['pic'] ?>" style="width: 80px;" row="<?= $i+1 ?>">
						</div>
						<div class="col-sm-4">
							<input type="hidden" class="b_pic_url" name="b_pic_url<?= $i+1 ?>" value="<?= $banner_pdt[$i]['pic'] ?>" row="<?= $i+1 ?>">
						...
						<div class="col-sm-4">
							<input class="form-control daterange" type="text" name="b_daterange<?= $i+1 ?>" value="<?= $dt_show[$banner_pdt[$i]['id']] ?>" readonly />
						</div>
...
<?php for ($l=0; $l < count($look_pdt); $l++) : ?>		
					<div class="form-group look<?= $l+1 ?>">
						...
						<div class="col-sm-2">
							...
							<input type="hidden" id="choose_look<?= $l+1 ?>" name="hid_products_id<?= $l+1 ?>" value="<?= $look_pdt[$l]['pdt_id'] ?>" row="<?= $l+1 ?>" />
						</div>
						<div class="col-sm-1">
						...
						<div class="col-sm-3">
							<input type="text" class="form-control" placeholder="輸入 YOUTUBE 網址" name="txt_url_youtube<?= $l+1 ?>" value="<?= $look_pdt[$l]['url_youtube'] ?>" row="<?= $l+1 ?>" pattern="^https:\/\/www.youtube.com\/embed\/.+" required />
						</div>
						<div class="col-sm-1">
							<button type="button" class="fa-check-circle btn btn-primary btn-flat check_url_youtube fa fa-check-circle " row="<?= $l+1 ?>" style="width: 31px;height: 34px;" ></button>
...
datetimepick();
	...
	$(document).on('click', '.btn-remove_banner', function() {
	...
	$(document).on('click', '.check_url_youtube', function() {
		$("#hid_row_youtube").val($(this).attr('row'));
		$('#div_url_pic'+$("#hid_row_youtube").val()+'').html('<iframe src="'+$('input[name=txt_url_youtube'+$("#hid_row_youtube").val()+']').val()+'"></iframe>');
	});
	...
	$('#btn_add_look').bind("click", function() {
		...
		$html += '<input type="hidden" id="choose_look'+l+'" name="hid_products_id'+l+'" value="" row="'+l+'" />';
		$html += '</div>';
		...
		$html += '<input type="text" class="form-control" placeholder="輸入 YOUTUBE 網址" name="txt_url_youtube'+l+'" value="" row="'+l+'" pattern="^https:\/\/www.youtube.com\/embed\/.+" required>';
		$html += '</div>';
		$html += '<div class="col-sm-1">';
	...
	$(document).on('click', '#btn_change_pdt', function() {
		...
		$.ajax({
			...
			complete: function(response){
				$('input[name=txt_url_youtube'+$("#hid_row_youtube").val()+']').val(response.responseText);
			}
		});
	});

	$(document).on('change', '.choose_theme', function() {
		$("#row_theme").val($(this).attr('row'));
		
		$.ajax({
			type: "POST",
			url: $('#hid_baseurl').val() + 'showmain/ThemeDateTime',
			cache: false,
			async : false,
			data: {'t':$(".choose_theme"+$("#row_theme").val()+"").val()},
			error: function(xhr){
				alert("Failure")
			},
			complete: function(response){
				$('input[name=t_daterange'+$("#row_theme").val()+']').val(response.responseText);
			}
		});
	});
	...
	$('#btn_add_banner').bind("click", function() {
		...
		$html += '<input type="url" class="form-control" name="txt_url'+i+'" value="" placeholder="請輸入整串網址"  >';
		$html += '</div>';
		$html += '<div class="col-sm-2">';
		$html += '<img name="banner_pic" src="" style="width: 80px;" row="'+i+'">';
		$html += '</div>';
		$html += '<div class="col-sm-4">';
		$html += '<input type="hidden" class="b_pic_url" name="b_pic_url'+i+'" value="" row="'+i+'">';
		...
		$html += '<div class="col-sm-4">';
		$html += '<input class="form-control daterange" type="text" name="b_daterange'+i+'" value="'+now_dt+' - '+now_dt+'" readonly />';
		$html += '</div>';
	...
	$(document).on('click', '.pchange', function() {
		$("#hid_row_pic").val($(this).attr('row'));
		key = $("#hid_row_pic").val();
<?php for ($i=0; $i < count($banner_pdt); $i++) : ?>
		if(key == <?= $i+1 ?>) {
...
<?php endfor; ?>
	...
	$("#btn_change_pdt").bind("click",function(){
		...
		$("input[name='hid_products_id"+$('#hid_look_pdt').val()+"'][row="+$('#hid_look_pdt').val()+"]").val($('#div_search :selected').val())
...
function add_theme_show() {
	if ($("#hid_row_theme").val() == 8) {
		$("#btn_add_theme").remove();
	}
}
